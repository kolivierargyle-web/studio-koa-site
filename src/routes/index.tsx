...
<section id="about" className="px-[clamp(0.44rem,2.3vw,33px)] py-[clamp(3.1rem,9.2vw,138px)]">
  <div className="mx-auto flex min-h-[199px] max-w-full items-center justify-center">
    <div className="text-center w-full">
      <p className="text-xl sm:text-3xl md:text-[4.65rem] font-bold leading-[1.1] max-w-[750px] md:max-w-[3000px]">
        Koa studio is a compact full-service<br className="hidden md:block" />creative direction & production studio.<br className="hidden md:block" />Concept to final cut. Photography, video &<br className="hidden md:block" />brand identity. We build the right team for every<br className="hidden md:block" />project, delivering impactful work without the<br className="hidden md:block" />cost or complexity of a large agency.
      </p>
    </div>
  </div>
</section>
...
<section className="w-full bg-black">
  <nav className="flex items-center justify-center px-[20px] sm:px-[40px] py-[16px] relative">
    <div className="flex gap-[8px] sm:gap-[32px] absolute left-[20px] sm:left-[40px]">
      <a href="#about" onClick={(e) => handleNavClick(e, 'about')} className="text-[10px] sm:text-sm md:text-lg text-white hover:text-[#BB95FF] cursor-pointer no-underline">About</a>
      <a href="#services" onClick={(e) => handleNavClick(e, 'services')} className="text-[10px] sm:text-sm md:text-lg text-white hover:text-[#BB95FF] cursor-pointer no-underline">Services</a>
    </div>
    <div className="flex gap-[8px] sm:gap-[32px]">
      <a href="#work" onClick={(e) => handleNavClick(e, 'work')} className="text-[10px] sm:text-sm md:text-lg text-white hover:text-[#BB95FF] cursor-pointer no-underline flex items-center gap-[4px]">Work <span>▼</span></a>
    </div>
    <div className="flex gap-[8px] sm:gap-[32px] absolute right-[20px] sm:right-[40px]">
      <a href="#spotlight" onClick={(e) => handleNavClick(e, 'spotlight')} className="text-[10px] sm:text-sm md:text-lg text-white hover:text-[#BB95FF] cursor-pointer no-underline">Creative spotlight</a>
      <a href="#contact" onClick={(e) => handleNavClick(e, 'contact')} className="text-[10px] sm:text-sm md:text-lg text-white hover:text-[#BB95FF] cursor-pointer no-underline">Contact</a>
    </div>
  </nav>
</section>
{/* Work grid */}
